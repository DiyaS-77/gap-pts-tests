...
from pts_report import generate_pts_xml_report
# Initialize logger
...
def run_pts_trigger(args):
    """
    Trigger execution of a PTS project remotely via gRPC,
    stream logs, collect results, and generate XML report.
    """
    ...
    for output in stub.RunProject(request):
        ...
        # Detect testcase start
        if "Running" in line and "/" in line:
            parts = line.split("Running")
            if len(parts) > 1:
                current_testcase = parts[1].strip()

        # Detect summary lines
        if ":" in line and "/" in line:

            # Example:
            # GAP/ADV/BV-03-C : INDCSV : Failed to scan advertising packets.
    ...
    # Generate XML report
    if results:
        report_file = generate_pts_xml_report(
            results=results,
            project=args.project
        )

        logger.info(f"PTS XML report generated: {report_file}")
