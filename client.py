...
def parse_testcases_argument(testcases_arg):
    """Parse testcases argument from CLI.

    Supports:
    --testcases TC1,TC2,TC3
    --testcases file:testcases.txt
    """
    if not testcases_arg:
        return []
    if testcases_arg.startswith("file:"):
        filepath = testcases_arg.split("file:", 1)[1]
        if not os.path.exists(filepath):
            logger.error("Testcase file not found: %s", filepath)
            sys.exit(1)
        with open(filepath) as f:
            return [line.strip() for line in f if line.strip()]
    return [tc.strip() for tc in testcases_arg.split(",") if tc.strip()]
...
def run_pts_trigger(args):
    ...
    testcase_list = parse_testcases_argument(args.testcases)
    logger.info("Selected testcases: %s", testcase_list if testcase_list else "ALL")
    ...
    download_implicit_log(stub, args)
...
def download_report(stub, args):
    ...
    save_dir = os.path.join(parent_dir, "reports")
...
def download_implicit_log(stub, args):
    logger.info("Downloading implicit_send_log.txt from Windows PTS server...")
    request = pts_trigger_pb2.ProjectRequest(
        config=args.config,
        testbed=args.testbed,
        workspace=args.workspace,
    ...
    response = stub.GetImplicitLog(request)
    if not response.content:
        logger.error("implicit_send_log.txt is empty or not found on server")
        return
    logger_instance = Logger("pts")
    session_path = logger_instance.logger.log_path
    save_path = os.path.join(session_path, response.filename)
    with open(save_path, "wb") as f:
        f.write(response.content)
    logger.info("implicit_send_log.txt saved at: %s", save_path)
...
def parse_args():
    ...
    pts_parser.add_argument(
        "--testcases",
        help="Comma-separated testcases OR file:<path>"
    )
