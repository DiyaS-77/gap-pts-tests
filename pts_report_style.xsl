...
def run_pts_trigger(args):
    ...
    stream logs, collect results, and generate XML reports.
    """
    ...
    for output in stub.RunProject(request):
        ...
        # Parse testcase summary line
        if " : " in output.line and "Total testcases" not in output.line:
    ...
    from pts_report import generate_pts_text_report

    report_file = generate_pts_text_report(args.project, results)

    logger.info(f"PTS report generated: {report_file}")
